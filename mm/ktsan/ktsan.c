...
void ktsan_print_diagnostics(void)
{
	...
	if (thr != NULL) {
		...
		kt_trace_restore_state(thr, thr->last_event_disable_time,
				       &state);
		kt_stack_print(&state.stack, 0);
		pr_err("\n");

		pr_err("Last event enable:\n");
		kt_trace_restore_state(thr, thr->last_event_enable_time,
				       &state);
		kt_stack_print(&state.stack, 0);
		pr_err("\n");
	}
...
static void ktsan_memblock_alloc(void *addr, unsigned long size,
